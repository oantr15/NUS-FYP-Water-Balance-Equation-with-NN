...
for basin_id in all_basins:
    ...
    def create_model(input_shape, seed, num_filters, model_type='hybrid'):
        ...
        if model_type == 'hybrid':
            ...
            cnn_output = ConvLayer(filters=2, kernel_size=10, padding='causal', seed=seed, name='Conv2')(cnn_output)
    ...
    gw_common= flow_common[:, :, 1:]
    flow_water_common= flow_common[:, :, 0:1]
    ...
    evaluate_set['flow_common'] = np.clip(flow_water_common[0, :, :], a_min = 0, a_max = None)
    ...
    evaluate_set_2['gw_common'] = np.clip(gw_common[0, :, :], a_min = 0, a_max = None)
    ...
    nse_common_flow = calc_nse(evaluate_set['flow_obs'].values, evaluate_set['flow_common'].values)
    ...
    nse_common_gw = calc_nse(evaluate_set_2['gw_obs'].values, evaluate_set_2['gw_common'].values)
    ...
    evaluation_plot(ax3, plot_set, 'flow_common', '#4daf4a', nse_common_flow, basin_id)
    ...
    evaluation_plot_2(ax6, plot_set_2, 'gw_common', '#4daf4a', nse_common_gw, basin_id)
